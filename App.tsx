...
const App: React.FC = () => {
  ...
  const historyStats = useMemo(() => {
    ...
    const dateGroups = userHistory.reduce((acc: Record<string, Task[]>, task) => {
      if (!acc[task.date]) acc[task.date] = [];
      acc[task.date].push(task);
      return acc;
    }, {} as Record<string, Task[]>);

    return Object.entries(dateGroups).map(([date, dayTasks]: [string, Task[]]) => {
  ...
  const handleAiSuggest = async () => {
    setIsAiLoading(true);
    const existingTopics = Array.from(new Set(userTasksToday.map(t => t.topic))) as string[];
